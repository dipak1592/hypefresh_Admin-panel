...
const ChartOne: React.FC = () => {
  const [state, setState] = useState<ChartOneState>({
    series: [
      {
        name: 'Comments', // Update the series name to "Comments"
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (timeFrame === 'monthly') {
          commentData = Array(12).fill(0);
          response.data.forEach((item: any) => {
            const month = new Date(item.start).getMonth();
            if (month >= 0 && month <= 11) {
              commentData[month] += item.comments; // Update to use item.comments
            }
          });
          ...
          commentData = response.data.map((item: any) => item.comments); // Update to use item.comments
        ...
        <div className="flex w-full flex-wrap gap-3 sm:gap-5">
          <div className="flex min-w-47.5">
            ...
            <div className="w-full">
              <p className="font-semibold text-primary">Total Comments</p> {/* Update to reflect "Comments" */}
          ...
          <div className="inline-flex items-center rounded-md bg-whiter p-1.5 dark:bg-meta-4">
            <button
              className={`rounded py-1 px-3 text-xs font-medium ${
                timeFrame === 'weekly' ? 'bg-white dark:bg-black shadow-card' : 'hover:bg-white hover:shadow-card'
            ...
            <button
              className={`rounded py-1 px-3 text-xs font-medium ${
                timeFrame === 'monthly' ? 'bg-white dark:bg-black shadow-card' : 'hover:bg-white hover:shadow-card'
              } dark:text-white dark:hover:bg-boxdark`}
